...
export const ResetPasswordContainer: FC<ResetPasswordContainerProps> = () => {
  ...
  >({
    mutationFn: async resetPasswordInput =>
      fetcher.post('/auth/reset-password', resetPasswordInput),
    onError: error => {
      const mappedError = ErrorMapper.mapErrorToApiResponse(error);

      for (const key in mappedError.errors) {
        const newKey = key as keyof ResetPasswordDto;

        setError(newKey, {
          type: 'manual',
          message: mappedError.errors[key][0]
        });
      }
    }
  });
  ...
  return (
    <>
      ...
      {mutation.isError && (
        ...
        >
          {ErrorMapper.mapErrorToApiResponse(mutation.error).message}
        </Alert>
      )}
